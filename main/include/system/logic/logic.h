        ...
        while (true)
        {
            ...
            if (mySerial)
            {
                ...
                for (size_t i = 0; i < bytes; i++)
                {
                    ...
                    {
                        if (rxInd < (sizeof(myRxBuffer) - 1)) { myRxBuffer[rxInd++] = c; }
                    }
                }
            }
        }
...
private:
    ...
    void mqttCallback(const std::string& topic, const std::string& data) noexcept
    {
        ...
        size_t copyLen = (data.length() < (RxLen - 1))
                             ? data.length()
                             : (RxLen - 1); // Checks data lenght to be smaller than RxLen else it
                                            // will be max size Rxlen.
        memcpy(mqttBuffer, data.data(), copyLen); // memory copy (minneskopiering)
                                                  // Its sole purpose is to copy a specified number
                                                  // of bytes from one location in RAM to another.
    ...
    void mqttTemp(bool force = false)
    {
        std::lock_guard<std::mutex> lock(
            myMqttMutex); // Locks the function so that we avoid Race Condition.
        ...
        if (force)
        {
            ...
            myMqtt->send(topic, reinterpret_cast<const std::uint8_t*>(tempBuffer),
                         strlen(tempBuffer));
    ...
    std::mutex
        mySerialMutex; //  Protects serial when more than one is writing to Uart/serial at once
};
} // namespace app::logic