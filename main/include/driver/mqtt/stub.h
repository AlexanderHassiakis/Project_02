...
public:
    ...
    {
        if (latestInstance == this) { latestInstance = nullptr; }
    }
              ...
              std::uint16_t byteLen) noexcept override
    {
        ...
        {
            std::printf("[MQTT STUB] SEND ERROR\n");
        }
    }
    ...
    void registerCallback(
        std::function<void(const std::string& topic, const std::string& data)> cb) noexcept override
    {
        myDataCallback = cb;
     ...
     */
    void simulateIncomingMessage(const std::string& topic, const std::string& data)
    {
        // Vi kollar efter latestInstance för att skicka till den stubbe som faktiskt har callbacken
        // registrerad!
